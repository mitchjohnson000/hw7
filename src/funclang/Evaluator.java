...
import funclang.AST.AssignExp;
import funclang.AST.DeRefExp;
import funclang.AST.FreeExp;
import funclang.AST.RefExp;
import funclang.Env.*;
...
public class Evaluator implements Visitor<Value> {
	...
	public Value visit(RefExp e, Env env) {
		Value value = (Value)e.exp.accept(this, env);
	...
	public Value visit(DeRefExp e, Env env) {
		...
		if(!( val instanceof Value.RefVal)) return new Value.DynamicError("Deref requires a RefVal");
		return heap.deref((Value.RefVal)val);
	}

	@Override
	public Value visit(AssignExp e, Env env) {
		Value val = (Value)e.exp.accept(this, env);
		if(!( val instanceof Value.RefVal)) return new Value.DynamicError("Assign requires a RefVal");
		Value value =(Value)e.exp2.accept(this, env);
		return heap.setref((Value.RefVal)val, value);
	}

	@Override
	public Value visit(FreeExp e, Env env) {
		Value val = (Value)e.exp.accept(this, env);
		if(!( val instanceof Value.RefVal)) return new Value.DynamicError("Free requires a RefVal");
		return heap.free((Value.RefVal)val);
	}
}
